...
{
    vec3 ambientColor = vec3(texture(textures[0],textureCoordinate))* lightAmbient;
	return ambientColor;
}
vec3 diffuse(vec4 lightPosition,vec3 lightDiffuse,inout vec3 lightDirection,inout vec3 normal)
{
	...
	vec3 diffuseColor = (difference*vec3(texture(textures[0],textureCoordinate)))* lightDiffuse;
	return diffuseColor;
}
vec3 specular(vec3 lightSpecular,vec3 lightDirection ,vec3 normal)
{
	...
	switch(texturesCount)
	{
		...
		case 2:
			...
			reflectDirection = reflect(-lightDirection, normal);  
			spec = pow(max(dot(viewDirection, reflectDirection), 0.0), material.shininess);
			specularColor = (spec *vec3(texture(textures[1],textureCoordinate)))* lightSpecular;
		break;
	}
...
vec3 emission()
{
	if(texturesCount == 3)
	{
		return vec3(texture(textures[2],textureCoordinate));
	}
	return vec3(0,0,0);
}
